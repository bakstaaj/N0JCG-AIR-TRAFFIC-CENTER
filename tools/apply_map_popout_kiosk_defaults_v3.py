#!/usr/bin/env python3
"""
Apply Map Pop-out Kiosk Defaults V3 for RTL-Windows-ADS-B-Tracker.

Frontend-only patch layered on top of Map Pop-out / Full Screen V1:
- In ?map_popout=1 mode, default the map view to receiver-centered 60-mile radius.
- Use the same radius as the kiosk radio/Airband radius default.
...
Supported URL switches:
  ?map_popout=1
      Default: receiver-centered 60-mile map/radio radius.
  ...
  ?map_popout=1&autofit=planes
      Alias for map_fit=planes.
  ?map_popout=1&map_radius_miles=90
      Receiver-centered map/radio radius override.
  ?map_popout=1&radius_miles=90
      Alias for map_radius_miles.
  ?map_popout=1&radius=90
      Alias for map_radius_miles.
  ?map_popout=1&radio_radius_miles=75
      Optional radio/Airband radius override when it should differ from the map radius.
...
JS_START = "// MAP_POPOUT_KIOSK_DEFAULTS_V3_START"
JS_END = "// MAP_POPOUT_KIOSK_DEFAULTS_V3_END"
...
def remove_known_blocks(text: str) -> str:
    changed = True
    while changed:
        changed = False
        for start, end in zip(OLD_START_MARKERS, OLD_END_MARKERS):
...
def main() -> int:
    try:
        ...
        if "MAP_POPOUT_FULLSCREEN_V1_START" not in js:
            print("WARNING: MAP_POPOUT_FULLSCREEN_V1_START not found. V3 can still apply, but install/apply the pop-out V1 patch first if the Pop Out Map button is missing.")

        cleaned = remove_known_blocks(js)
        backup = APP_JS.with_suffix(APP_JS.suffix + f".bak_map_popout_kiosk_defaults_v3_{_dt.datetime.now().strftime('%Y%m%d_%H%M%S')}")
        backup.write_text(js, encoding="utf-8", newline="\n")
        APP_JS.write_text(cleaned.rstrip() + "\n\n" + JS_BLOCK, encoding="utf-8", newline="\n")

        print(f"PASS: Applied map pop-out kiosk defaults V3 to {APP_JS}")
        print(f"PASS: Backup written to {backup}")
        print("PASS: Pop-out default is now receiver-centered 60-mile radius")
        print("PASS: Pop-out radio/Airband radius follows the kiosk radius by default")
