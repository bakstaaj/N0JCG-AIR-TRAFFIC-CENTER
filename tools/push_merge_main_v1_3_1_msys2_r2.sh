...
# R2: accepts the actual active trail V3 marker used by the working patch
#     (ACTIVE_TRAIL_CLEANUP_V3), not only ACTIVE_TRAIL_CLEANUP_V3_START.
...
COMMIT_MESSAGE="${COMMIT_MESSAGE:-Release ${RELEASE_TAG} active trail cleanup and map popout kiosk defaults}"
TAG_MESSAGE="${TAG_MESSAGE:-Release ${RELEASE_TAG} active trail cleanup and map popout kiosk defaults}"
SKIP_TAG="${SKIP_TAG:-0}"
...
# Feature marker validation.
# Active trail V3 may be installed as either an append wrapper with ACTIVE_TRAIL_CLEANUP_V3_START
# or as a targeted inline patch marked ACTIVE_TRAIL_CLEANUP_V3. The inline form is the working
# behavior that tied marker removal to active trail layer removal.
has_marker 'ACTIVE_TRAIL_CLEANUP_V3' || fail "missing active trail cleanup V3 marker in web/app.js"
has_marker 'removeTrailLayersForAircraft' || fail "missing trail-layer removal function reference in web/app.js"
has_marker 'MAP_POPOUT_FULLSCREEN_V1_START\|MAP_POPOUT_KIOSK_DEFAULTS_V2_START\|MAP_POPOUT_KIOSK_DEFAULTS_V3_START' || fail "missing map popout/fullscreen marker in web/app.js"
has_marker 'MAP_POPOUT_KIOSK_DEFAULTS_V3_START' || fail "missing map popout kiosk defaults V3 marker in web/app.js"
has_marker 'DEFAULT_RECEIVER_RADIUS_MILES = 60' || fail "missing 60-mile kiosk default in web/app.js"
pass "feature markers verified"

# Run feature-specific validators if available. These remain authoritative when present,
# but the script no longer fails merely because the active V3 marker uses the inline form.
...
if git show-ref --verify --quiet "refs/remotes/${REMOTE_NAME}/${MAIN_BRANCH}"; then
  ...
  pass "${MAIN_BRANCH} fast-forwarded or already aligned with ${REMOTE_NAME}/${MAIN_BRANCH}"
else
...
run node --check web/app.js
has_marker 'ACTIVE_TRAIL_CLEANUP_V3' || fail "post-merge missing active trail cleanup V3 marker"
has_marker 'MAP_POPOUT_KIOSK_DEFAULTS_V3_START' || fail "post-merge missing map popout kiosk defaults V3 marker"
has_marker 'DEFAULT_RECEIVER_RADIUS_MILES = 60' || fail "post-merge missing 60-mile kiosk default"
...
# Create/push tag without force-moving an existing tag.
if [[ "$SKIP_TAG" == "1" ]]; then
  info "SKIP_TAG=1; skipping tag creation/push"
else
  head_sha="$(git rev-parse HEAD)"
  remote_tag_sha="$(git ls-remote --tags "$REMOTE_NAME" "refs/tags/${RELEASE_TAG}" | awk '{print $1}' | head -n 1)"

  if git rev-parse -q --verify "refs/tags/${RELEASE_TAG}" >/dev/null; then
    tag_sha="$(git rev-list -n 1 "$RELEASE_TAG")"
    if [[ "$tag_sha" != "$head_sha" ]]; then
      fail "local tag ${RELEASE_TAG} already exists at ${tag_sha}, but ${MAIN_BRANCH} HEAD is ${head_sha}. Choose a new tag or fix the existing tag intentionally."
    fi
    pass "local tag ${RELEASE_TAG} already points to ${MAIN_BRANCH} HEAD"
  else
    if [[ -n "$remote_tag_sha" && "$remote_tag_sha" != "$head_sha" ]]; then
      fail "remote tag ${RELEASE_TAG} already exists at ${remote_tag_sha}, but ${MAIN_BRANCH} HEAD is ${head_sha}. Choose a new tag."
    fi
